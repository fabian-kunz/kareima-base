...
export interface KTableHeader {
  ...
  formatter?: (value: unknown, item: KTableRow) => string;
  renderAs?: "text" | "chip" | "chip-list" | "stacked";
  subtitleAccessor?: (value: unknown, item: KTableRow) => string;
  chipColorMap?: Record<string, string>;
  chipColorResolver?: (value: unknown, item: KTableRow) => string;
  sortable?: boolean;
  sortAccessor?: (value: unknown, item: KTableRow) => string | number;
}
...
type KTableActionsSource =
  | KTableAction[]
  | ((item: KTableRow) => KTableAction[]);
...
const props = withDefaults(
  defineProps<{
    ...
    rowActions?: KTableActionsSource;
    clickableRows?: boolean;
    rowClass?: (item: KTableRow) => string;
    expandableRows?: boolean;
    canExpandRow?: (item: KTableRow) => boolean;
...
const emit = defineEmits<{
  ...
  (e: "row-click", item: KTableRow): void;
}>();
...
function rowKey(item: KTableRow, index: number): string {
...
function toggleSort(header: KTableHeader) {
...
function formatCellValue(header: KTableHeader, item: KTableRow): string {
  const value = asRecord(item)[header.key];
  if (header.formatter) {
...
function isExpanded(item: KTableRow): boolean {
  return expandedRowKeys.value.has(rowKey(item, -1));
}

function onRowClick(item: KTableRow) {
